...
export const sendPlanEmail = onCall(
  ...
  async (request) => {
    ...
    if (!request.auth || !request.auth.token.email) {
      throw new HttpsError(
        "unauthenticated",
        "You must be signed in with a valid email address to send daily plans."
      );
    }

    const email = request.auth.token.email;
    const { date, tasks } = request.data as {
      date: string;
      tasks: Task[];
    };
    ...
    if (!date || !tasks || !Array.isArray(tasks)) {
      throw new HttpsError(
        "invalid-argument",
        "Missing or invalid parameters: date and tasks are required."
      );
    }
    ...
    if (!resendApiKey) {
      throw new HttpsError(
        "failed-precondition",
        "Resend API key is not configured on the server."
      );
    }
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        err.message || "Failed to dispatch email via Resend."
      );
    }
  }
);
