...
exports.sendPlanEmail = void 0;
const https_1 = require("firebase-functions/v2/https");
const app_1 = require("firebase-admin/app");
const resend_1 = require("resend");
(0, app_1.initializeApp)();
exports.sendPlanEmail = (0, https_1.onCall)({ secrets: ["RESEND_API_KEY"], cors: true }, async (request) => {
    // 1. Verify Authentication
    if (!request.auth || !request.auth.token.email) {
        throw new https_1.HttpsError("unauthenticated", "You must be signed in with a valid email address to send daily plans.");
    }
    const email = request.auth.token.email;
    const { date, tasks } = request.data;
    // 2. Validate Inputs
    if (!date || !tasks || !Array.isArray(tasks)) {
        throw new https_1.HttpsError("invalid-argument", "Missing or invalid parameters: date and tasks are required.");
    }
    ...
    if (!resendApiKey) {
        throw new https_1.HttpsError("failed-precondition", "Resend API key is not configured on the server.");
    }
    ...
    const renderTasksSection = (title, icon, timeLabel, items) => {
        if (items.length === 0)
            return "";
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", err.message || "Failed to dispatch email via Resend.");
    }
});
